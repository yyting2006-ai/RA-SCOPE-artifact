...
\title{RA-SCOPE: An Auditable Zero-Parameter-Fusion Ensemble for Ordinal
...
\IEEEauthorblockA{Beijing Language and Culture University\\
\textsuperscript{1}Hainan International College \quad
\textsuperscript{2}School of Information Science\\
Beijing, China\\
...
monotone experts (length, character n-gram, Standard-derived measurements, and a
fine-tuned Chinese MacBERT) are combined by an unweighted probability pool that
introduces no trainable fusion parameters. A label-free source-distance support
rank licenses a closed-form Euclidean projection of the pooled logits onto a
retained-margin ordinal set. The projection is applied only when the two
complementary anchors agree and at least one of them is backed by source
evidence. Every item therefore carries an explicit state (CONSENSUS, CONFLICT,
or NO-SUPPORT) that a human review workflow can act on. On 120 independently
triple-annotated and adjudicated Chinese picturebooks, RA-SCOPE reaches .823
quadratic weighted kappa and .417 MAE, improving on fine-tuned MacBERT by .060
and on a Qwen2.5-3B rubric baseline by .285 with paired 95\% intervals that
exclude zero, and it halves the rate of errors exceeding one level. It retains
.786 QWK and .393 MAE with no errors above one level on a separately
adjudicated 28-book collection. A post-hoc stacked logistic fusion reaches .844
QWK and .277 MAE where target labels exist, trading the zero-parameter property
and the preservation guarantee for lower absolute error. Every activated
consensus grade is preserved by construction, and every decision can be
re-executed by hand.
\end{abstract}
...
educational-measurement decision. Publishers, reading programs and
...
Standards for International Chinese Language Education~\cite{cllc2024standards})
rather than by preference.

Today the decision is made by expert panels: the collections used in this study
were graded by three annotators with International Chinese Education training
who each contributed approximately 20~hours, followed by adjudication by a
fourth. The cost of that process does not scale with catalog growth, and its
output is fragile across sources: books that look comparable in length are
graded differently by different publishers because layout, illustration and
annotation conventions differ sharply.
...
overturn. That question, rather than the pooling arithmetic, is what determines
...
solution and no fitted fusion parameters. Fig.~\ref{fig_dataflow} summarizes the
design.
...
counter-length pairs, Standard-derived construct alignment,
observable-modality routing, and a learned-fusion comparison that quantifies
...
placed in front of, or behind, an existing human review queue: Section~\ref{sec_deploy}
sets out the resulting triage behavior and its measured cost.

%=====================================================================
\section{Related Work}
%=====================================================================

\subsection{Readability and Ordinal Modeling}
...
modeling~\cite{li2024going}. We focus on inference-time composition and
...
(Table~\ref{tab_collections}). Annotators P01--P03 independently annotated
randomized spreadsheets; P04 then adjudicated disagreements and checked the
...
(\ref{eq_pool}) and the projected probabilities
...
$0 < \lambda \le 1$, (\ref{eq_clip}) is the unique Euclidean projection in
(\ref{eq_proj}), and its point grade is $g_i^{\star}$.}
...
(\ref{eq_proj}). Every prefix bound is non-negative and every suffix bound is
negative, preserving all threshold signs and therefore $g_i^{\star}$.
\hfill$\blacksquare$

\begin{figure*}[!t]
\centering
\resizebox{\textwidth}{!}{\input{figs/fig1_dataflow.tex}}
\caption{RA-SCOPE dataflow. The source-development collection trains the
experts and calibrates the support rank. Four monotone experts form a
parameter-free uniform pool. The decision contract first checks whether at least
one anchor is source-supported, then whether the length and character grades
agree; grade agreement always involves both anchors, and support from a single
anchor can license an existing agreement but never create one. Supported
...
(Table~\ref{tab_main}). Two properties matter more for deployment than the
...
\begin{table}[t]
\caption{Global Grading. The support-disabled ablation applies projection on
full anchor agreement without the support condition; it gives identical point
predictions to RA-SCOPE on both collections.}
\label{tab_main}
\centering
\footnotesize
...
Support disabled       & .823 & .417 & .592 & .008 & .786 & .393 \\
\bottomrule
\end{tabular}
\end{table}
...
grades (Table~\ref{tab_main}, final row); requiring both anchors to be supported gives
...
test this directly.
Using the four experts' continuous ordinal scores from the frozen artifact
(four features per book: length, character TF--IDF, Standard-derived
measurements, and fine-tuned MacBERT), we fit a multinomial logistic regression
on the 120-book collection. Folds are grouped by original-work lineage,
matching the main protocol; every lineage holds a single evaluated book on this
collection, so grouped and stratified folds coincide. We repeat the
cross-validation over ten seeds at $k\in\{4,5,6,10\}$ and report the mean over
the ten seeds at $k{=}5$, which Table~\ref{tab_stack} gives. Learned fusion is
competitive on QWK and clearly better on absolute error. Its QWK is .844 against
.823, a difference of $+.021$ (95\% book-bootstrap CI $[-.026,+.071]$, that is,
not distinguishable at this sample size), while its MAE is .277 against .417, a
difference of $-.140$ (CI $[-.248,-.035]$), and its catastrophic-error rate is
.001 against .008. The advantage is stable across all four fold counts, with QWK
between .844 and .848.
...
We therefore read Table~\ref{tab_stack} as the measured price of auditability on
...
the mean over ten seeds of a lineage-grouped cross-validation at $k{=}5$; the
difference row is the difference of the two rows above.}
\label{tab_stack}
\centering
\footnotesize
...
Stacked logistic (5-fold CV)     & \textbf{.844} & \textbf{.277} & \textbf{.724} & \textbf{.001} \\
\midrule
Difference                       & $+.021$ & $-.140$ & $+.132$ & $-.007$ \\
\bottomrule
\end{tabular}
\end{table}
...
After residualizing both variables for log character count, log page count and
...
pooled projection is a coordinate-wise clip (Eq.~\ref{eq_clip}) and can be
re-executed by a reviewer by hand.

\noindent\textbf{Triage.} Accuracy stratifies by state. On the main collection,
CONSENSUS items are exact 71.1\% of the time and are never more than one level
away ($n{=}38$, within-one accuracy 1.000), whereas CONFLICT and NO-SUPPORT
items are exact 56.5\% and 50.0\% of the time ($n{=}46$ and $n{=}36$). A
workflow can therefore accept CONSENSUS grades under a one-level tolerance and
route CONFLICT and NO-SUPPORT items to human adjudication, which concentrates
scarce expert time on 82 of 120 books. Nothing is hidden: every item still
receives an ordinal distribution, and the state is reported alongside it.

\noindent\textbf{Preservation.} The guarantee in Proposition~2 is what makes this
routing defensible. When the two anchors agree and at least one is
...
Section~\ref{sec_stack} cannot be constructed at all until adjudicated books
...
errors relative to that pool (seven improve, two worsen), and this is enough for
...
comparisons, while the Qwen rubric baseline shows substantial platform
...
Zero-parameter pooling is a design requirement, not a convenience. Source
platforms differ sharply in layout, length and illustration conventions. A
learned fusion layer must be fitted on labeled books, so it carries its
fitting-platform label distribution into the next platform, and its per-item
weights are not inspectable. RA-SCOPE's fusion layer is itself unfitted: given
the four expert chains, Eq.~\ref{eq_clip} can be re-executed and audited item by
item, which a fitted weight vector does not permit. Where a fitted alternative
is affordable it does raise average fit: the stacked fusion of
Section~\ref{sec_stack} reduces MAE by .140 relative to the contract. Our claim
concerns decision guarantees and auditability under platform shift, not the best
average fit on a labeled target collection.
...
preserve applicable attribution and license conditions. The Standard may
...
separate robustness evaluations it improves or matches the strongest individual
experts and the uniform pool while remaining stable across support and margin
settings.
...
Entrepreneurship Training Program (No.~202610032033). The reproducibility
artifact (the projection module, its invariant tests, every per-book
prediction, the reported tables and the figure sources) is available at
\url{https://github.com/yyting2006-ai/RA-SCOPE-artifact}.
